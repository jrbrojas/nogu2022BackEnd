...
const getListTipoDocumento = async (req, res) => {
    try {
        const respConNS = await con.query("select * from public.tipo_documento", async (err, resp) => {
            logger.error(err);
            if (err) {
                logger.error(err)
    ...
    try {
        ...
        if (data != null) {
            ...
            var sql = "select ns.numero_servicio, " +
                ...
                " where (e.razonsocial like upper('%" + data.models.nombre + "%') or concat(b.nombres, ' ', b.apellidos) like upper('%" + data.models.nombre + "%')) " + numeFil + " order by ns.numero_servicio desc";
            const respConNS = await con.query(sql, async (err, resp_basicos) => {
                logger.error(err);
                if (err) {
                    return res.status(404).json({ data: [], mensaje: "Error al consultar datos", statuscode: 500 });
                }
                return res.status(200).json({ data: resp_basicos.rows, mensaje: "Lista registros", statuscode: 200 });
            });
                        ...
                        const respConNS = await con.query(select_contribuyentes, async (err, resp_select_contribuyentes) => {
                            ...
                            if (resp_select_trasmite_opciones.rows[0].tipo_tramite == 2) {
                                const respConNS = await con.query(select_bien, async (err, resp_select_bien) => {
                                    resp.push(resp_datos_entidad.rows[0]);
                                    resp.push(resp_select_datos_entidad_uif.rows[0]);
                                    resp.push(resp_select_trasmite_opciones.rows[0]);
                                    resp.push(resp_select_bien.rows[0]);
                                    resp.push(resp_select_contribuyentes.rows);
                                    return res.status(200).json({ data: resp, mensaje: "datos", statuscode: 200 });
                                });
                            } else {
                                resp.push(resp_datos_entidad.rows[0]);
