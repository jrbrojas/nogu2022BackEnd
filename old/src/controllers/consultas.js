...
exports.insert_tamite_opciones = async (data, numero_servicio) => {
    var fecha = new Date();
    var newFecha =
        fecha.getMonth() + 1 +
        "/" +
        ...
        "/" +
        fecha.getFullYear();
    var campos = "(tipo_persona, tipo_opcion, tipo_tramite, tipo_condicion, created_at, numero_servicio)";
    var valores = data.tipo_persona + ", " +
        data.tipo_opcion + ", " +
        data.tipo_tramite + ", " +
        data.tipo_condicion + ", " +
        "'" + newFecha + "', " +
        numero_servicio;
    return sql = "insert into public.trasmite_opciones " + campos + " values (" + valores + ")";
};

exports.insert_datos_laborales = async (data, numero_servicio) => {
    var campos = "(profesion,centro_trabajo,funcion_publica,cargo_publico,familiar_cargo,familiar_nombres,familiar_car,id_familiar_pais,informar_uif,oficial_cumplimiento,of_haberes,of_prestamos_familiares,numero_servicio,of_venta_bien_inmieble,of_rentas,of_donacion,of_donacion_pago,of_prestamos_bancario,of_herencia,of_venta_vehiculo,of_comercio,of_otros_describir,of_otros_describir_text,mp_efectivo,mp_deposito_cuenta,mp_bien_inmueble,mp_cheque,mp_transferencia_bancaria,mp_bien_mueble,mp_otros_describir,mp_otros_describir_text)";
    var valores = "'" + data.profesion + "', " +
        "'" + data.centro_trabajo + "', " +
    ...
    var sql = "insert into public.datos_laborales " + campos + " values (" + valores + ")";
    console.log(sql);
    return sql;
};

exports.insert_datos_apoderado = async (data, numero_servicio) => {
    var campos = "(num_documento, id_tipo_documento, otro_documento, nombres, apellidos, domicilio, nacionalidad, fecha_nacimiento, id_estado_civil, profesion, registro_poder, correo, conyugue_nombres, numero_servicio)";
    var valores = "'" + data.num_documento + "', " +
        ...
        numero_servicio;
    return sql = "insert into public.datos_apoderado " + campos + " values (" + valores + ")";
};

exports.insert_datos_basicos = async (data, numero_servicio) => {
    var campos = "(num_documento, id_tipo_documento, otro_documento, distrito, nombres, apellidos, pais_origen, ciudad, fecha_nacimiento, id_pais_nacionalidad, id_estado_civil, separa_patrimonio, partida, sede, cuidad_origen, domicilio, telefono_uno, telefono_dos, correo_electronico, nombre_conyugue, extranjero, id_ubigeo, numero_servicio)";
    var valores = "'" + data.num_documento + "', " +
        "" + data.id_tipo_documento + ", " +
        "'" + data.otro_documento + "', " +
        "'" + data.distrito + "', " +
       ...
       (this.formato_fecha(data.fecha_nacimiento) == null ?  ("" + null + ", ") : ("'" + this.formato_fecha(data.fecha_nacimiento) + "', ")) +
        ...
        "" + data.id_ubigeo + ", " +
        "" + numero_servicio;
    return sql = "insert into public.datos_basicos " + campos + " values (" + valores + ")";
};

////////////////////7
exports.insert_datos_bien = async (data, numero_servicio) => {
    var campos = "(tipo_bien, tipo_transferencia, num_placa, moneda, valor, targeta_propiedad, numero_servicio, mp_compra_venta, mp_donacion, mp_permuta, mp_dacion_pago, mp_otro, mp_otros_describir, numero_tarjeta_propiedad, mp_anticipo_legitima)";
    var valores = "'" + data.tipo_bien + "', " +
    ...
    return sql = "insert into public.datos_bien " + campos + " values (" + valores + ")";
};
//////////////////

//RUCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC

exports.insert_datos_entidad_uif = async (data, numero_servicio) => {
    var campos = "(obigado_informar, designo_oficial_cumplimiento, expuesta25, nombres, cargo, id_pais, of_giro_negocio, of_prestamos_socios, of_venta_bien_inmueble, of_intermediacion_financiera, of_prestamo_bancario, of_prestamo_terceros, of_venta_activos, of_otros_escribir_of, of_otros_escribir_of_text, mp_efectivo, mp_deposito_cuenta, mp_bien_inmueble, mp_cheque, mp_transferencia_bancaria, mp_bien_mueble, mp_otros_describir, mp_otros_describir_text, numero_servicio)";
    var valores = "" + data.obigado_informar + ", " +
        ...
        "'" + data.mp_otros_describir_text + "', " +
        "" + numero_servicio;
    return sql = "insert into public.datos_entidad_uif " + campos + " values (" + valores + ")";
};

exports.insert_datos_entidad = async (data, numero_servicio) => {
    var campos = "(ruc, razonsocial, direccion, telefono, correo, principal_contribuyente, declaracion_jurada, comercial, industrial, construccion, transporte, pesca, intermediacion_financiera, hoteles_restaurantes, agricultura, ensenanza, suministro_electricidad_gas, partida_registral, sede_registral, otros_describir, otro_opcion, numero_servicio)";
    var valores = "'" + data.ruc + "', " +
        "'" + data.razonsocial + "', " +
        "'" + data.direccion + "', " +
        "'" + data.telefono + "', " +
        "'" + data.correo + "', " +
        ...
        "" + numero_servicio;
    return sql = "insert into public.datos_entidad " + campos + " values (" + valores + ")";
};

exports.insert_contribuyentes = async (data, numero_servicio) => {
    var campos = "(id_tipo_documento, otro_documento, numero_documento, nombres, fecha_nacimiento, id_pais_nacionalidad, id_estado_civil, domicilio, profecion_ocupacion, telefono, inscripcion_registral, numero_servicio)";
    var valores = "" + data.id_tipo_documento + ", " +
        "'" + data.otro_documento + "', " +
        ...
        "" + numero_servicio;
    return sql = "insert into public.contribuyentes " + campos + " values (" + valores + ")";
};
...
exports.select_datos_entidad = async (numero_servicio, tipoQuery) => {
    return sql = 'select ' +
        ...
        'otro_opcion, ' +
        'numero_servicio ' +
        'from public.datos_entidad where numero_servicio = ' + numero_servicio;
};
